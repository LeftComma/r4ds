...
However, when a chunk is saved it only saves that chunk. This can cause issues if that chunk depends on previous ones, as these are not saved. You can fix this with the dependson option, that looks like this:
```{r raw_data}
